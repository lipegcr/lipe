...
//|                                                                  |
//+------------------------------------------------------------------+
string   Store_FileName1 = "FDlast_buy" + Symbol() + IntegerToString(PeriodSeconds() / 60);          // File name for storing probabilities, which determined in the results of virtual trade
string   Store_FileName2 = "FDlast_sell" + Symbol() + IntegerToString(PeriodSeconds() / 60) ;          // File name for storing probabilities, which determined in the results of virtual trade
...
//cria um arquivo csv
 filecsvcreat(Store_FileName1);
 filecsvcreat(Store_FileName2);
...
//|                                                                  |
//+------------------------------------------------------------------+
void filecsvcreat(string aFileName) {
 int h = FileOpen( aFileName, FILE_WRITE | FILE_ANSI | FILE_CSV);
 if(h == INVALID_HANDLE) {
...
void Push(int aPattern, double aValue) {
 for(int i = 1; i <= NN - 1; i++) { //3
  MarketPattern[NN + 1 - i][aPattern] = MarketPattern[NN - i][aPattern]; } //3
 ...
 if(SaveHistory) {
  if(!MQL5InfoInteger(MQL5_TESTING)) {
   if(Store_ChangesCount > 10 && TimeCurrent() >= Store_LastSaveTime) { //1
    Store_ChangesCount = 0;
    WriteDataToFile(Store_FileName1);
    WriteDataToFile(Store_FileName2); } //1
  } } }
...
void SaveHistoryInTester() {
 if(SaveHistory) {
  if(MQL5InfoInteger(MQL5_TESTING)) {
   WriteDataToFile(Store_FileName1);
   WriteDataToFile(Store_FileName2); } } }
...
//+------------------------------------------------------------------+
void WriteDataToFile(string aFileName) {
 int h = FileOpen(aFileName, FILE_CSV | FILE_WRITE | FILE_COMMON);
...
void ReadHistoryFile() {
 if(!Store_IsLoaded) { //1
  Store_IsLoaded = true;
  if(ReadHistory) {
   ReadDataFromFile(Store_TypesCount, Store_FileName1);
   ReadDataFromFile(Store_TypesCount, Store_FileName2); } } //1
}

//+------------------------------------------------------------------+
//| Reading a file with data for learning                            |
//+------------------------------------------------------------------+
void ReadDataFromFile(int & aTypesCount, string aFileName) {
 int h = FileOpen(aFileName, FILE_CSV | FILE_READ | FILE_COMMON);
 Store_LastSaveTime = (datetime)FileReadNumber(h);
 aTypesCount = (int)FileReadNumber(h);
 for(int is = 1; is <= Nstop; is++){
  for(int ip = 1; ip <= Nidelt; ip++){
   for(int ti = 0; ti <= aTypesCount - 1; ti++){
